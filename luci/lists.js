...
return view.extend({
	...
	render: function (data) {
		...
		o = s.option(form.DynamicList, 'preset', _('Готовые списки'),
			_('Работает вместе со своим, записи объединяются. Список разработчика приносит и подсети, чужие списки только доменные. Чужие списки никем не проверяются.'));
		o.value('byway', _('byway — основной список разработчика'));
		o.value('itdoginfo-geoblock', _('itdoginfo: сервисы, закрытые для РФ'));
		...
		/* ── Направления ───────────────────────────────────────────────
		   «Этот список -- в эту ноду». Отдельная секция UCI на каждое, имя
		...
		var rs = m.section(form.GridSection, 'route', _('Направления'),
			_('Свой список для отдельного ключа: перечисленное в нём пойдёт не в основную ноду, а в выбранную. Имя направления — латиницей, оно же имя файла со списком.'));
		rs.addremove = true;
		rs.anonymous = false;
		rs.sortable = false;
