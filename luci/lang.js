...
var DICT = {
	en: {
		...
		"byway понимает vless, vmess, trojan, shadowsocks и socks. Автоматически — Xray-core замеряет задержку и ведёт трафик через самый быстрый живой ключ.": "byway understands vless, vmess, trojan, shadowsocks and socks. Automatic — Xray-core measures the latency and routes through the fastest key that answers.",
		"Один ключ": "One key",
		...
		"Кусок конфигурации Xray-core объектом: protocol, settings, при нужде streamSettings. Тег byway подставит сам.": "A piece of Xray-core config as an object: protocol, settings, streamSettings if needed. byway adds the tag itself.",
		...
		"Запасной DNS-сервер": "Backup DNS server",
		"Спрашивается, когда первый молчит. Берите ДРУГОГО оператора: у одного оператора оба адреса лежат в одной сети и падают вместе. Отключить — вписать none.": "Asked when the first one stays silent. Pick a DIFFERENT operator: one operator's two addresses sit in the same network and fail together. To turn it off, type none.",
		"без запасного": "no backup",
		"Кем разрешать имя DNS-сервера": "What resolves the DNS server's name",
		"Нужен, только если резолвер задан ИМЕНЕМ, а не адресом: имя самого сервера разрешать больше некем. Спрашивают у него ровно это имя, остальное идёт по DoH. Пусто — значит резолверы задаются адресами.": "Needed only when the resolver is given by NAME rather than address: there is nothing else left to resolve the server's own name with. It is asked exactly that name, everything else goes over DoH. Empty means resolvers are given as addresses.",
		"Яндекс — 77.88.8.8": "Yandex — 77.88.8.8",
		...
		"Роутер ходит к сайтам из списка сам": "The router reaches the listed sites on its own",
		"Перехват ловит только домашние устройства, поэтому сам роутер до сайтов из списка не дотягивается. Включите, если это нужно — например чтобы обновления пакетов шли через туннель. Обновлениям самого byway настройка не нужна: они ходят через прокси. Проверено на одном роутере, поэтому по умолчанию выключено.": "Interception only catches home devices, so the router itself cannot reach the listed sites. Turn it on if you need that - for package updates through the tunnel, say. byway's own updates do not need it: they go through the proxy. Verified on one router, so it is off by default.",
		"Порт для трафика роутера": "Port for the router’s own traffic",
		...
		"Работает вместе со своим, записи объединяются. Списки byway и itdoginfo-subnets приносят подсети, остальные только домены. Чужие списки никем не проверяются, а подсети в них бывают широкими — byway скажет насколько.": "Works alongside your own list, the entries are merged. The byway and itdoginfo-subnets lists bring subnets too, the rest are domains only. Third-party lists are checked by nobody, and their subnets can be wide — byway will say how wide.",
		"byway — список разработчика": "byway — the author’s list",
		"itdoginfo — закрытое для РФ": "itdoginfo — closed to Russia",
		"itdoginfo — заблокированное в РФ": "itdoginfo — blocked in Russia",
		"itdoginfo — подсети сервисов и хостеров": "itdoginfo — subnets of services and hosters",
		...
		"Сюда вставляется текст выгрузки целиком, вместе с шапкой «# byway export».": "Paste the whole export text here, header «# byway export» included.",
		"сюда вставляется текст выгрузки": "paste the text of an export here",
		"Оставить нынешний ключ": "Keep the current key",
		"Принять всё, кроме подключения к VPN. По умолчанию ключ берётся из выгрузки — если он в ней есть.": "Accept everything except the VPN connection. By default the key is taken from the export -- if it has one.",
		"Принять": "Import",
		...
		"Выключено намеренно: обновление перезапускает службу, то есть на минуту отнимает туннель у всего дома. Включив, вы соглашаетесь, что это произойдёт в назначенный час ниже, по часам роутера, и не раньше чем через трое суток после выхода версии — важные ставятся сразу. Обновление идёт только внутри минорной версии, а если туннель не поднялся за минуту, byway сам возвращает прежнюю. След утром — строка «ночью …» в сводке.": "Off on purpose: an update restarts the service, taking the tunnel away from the whole house for a minute. By turning it on you agree that this happens at the hour set below, by the router’s clock, and no sooner than three days after a release appears — important ones go in at once. Updates stay within the minor version, and if the tunnel does not come up within a minute byway puts the previous one back. The morning trace is the «overnight …» line in the summary.",
		"Час обновления": "Update hour",
		"По часам роутера, а не по вашим: у прошивки из коробки пояс UTC. Какой это час у вас — скажет byway doctor строкой про пояс.": "By the router’s clock, not yours: stock firmware keeps time in UTC. Which hour that is for you is what byway doctor tells you in its time zone line.",
		"Проверить": "Check",
		...
		"Только для режима автовыбора: с каким шагом Xray-core проверяет задержку до каждого ключа. Чаще — быстрее заметит отвал, но больше лишнего трафика.": "For the automatic mode only: how often Xray-core checks the latency to every key. More often means an outage is noticed sooner, but there is more idle traffic.",
		"Язык": "Language",
		...
		"Движок Xray-core": "Xray-core engine",
