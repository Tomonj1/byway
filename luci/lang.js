...
var DICT = {
	en: {
		...
		"Перехват ловит только домашние устройства, поэтому сам роутер до сайтов из списка не дотягивается. ⚠️ Пока не включайте: в нынешнем виде заворот зацикливается и через несколько минут движок перестаёт принимать соединения. Обновлениям byway он не нужен — они ходят через прокси.": "Interception only catches home devices, so the router itself cannot reach the listed sites. ⚠️ Leave this off for now: as it stands the redirect loops, and within minutes the engine stops accepting connections. byway updates do not need it — they go through the proxy.",
