...
return view.extend({
	...
	render: function (data) {
		...
		o = s.option(form.Flag, 'router_via_vpn',
			...
			_('Перехват ловит только домашние устройства, поэтому сам роутер до сайтов из списка не дотягивается. ⚠️ Пока не включайте: в нынешнем виде заворот зацикливается и через несколько минут движок перестаёт принимать соединения. Обновлениям byway он не нужен — они ходят через прокси.'));
		o.default = '0';
		o.rmempty = false;
