...
function connName(statusText) {
	...
	/* В режиме автовыбора одной ноды нет: status пишет «подключение» и
	   «сейчас». В обычном — «нода» и «транспорт». Читаем оба вида. */
	if (g[_('подключение')])
		return g[_('подключение')] + (g[_('сейчас')] ? _('  ·  сейчас ') + g[_('сейчас')] : '');
	return (g[_('нода')] || _('(ключ не задан)')) +
...
return view.extend({
	...
	render: function (data) {
		...
		o = s.option(form.ListValue, 'on_failure', _('Если VPN не поднялся'),
			_('Нода не отвечает, ключ устарел. Пустить напрямую — дом остаётся с интернетом, но трафик для VPN идёт мимо него. Не пускать — закроется то, что ходило через VPN: в режиме «по спискам» это список, в режиме «всё через VPN» — весь выход наружу. Вход в роутер не задет.'));
